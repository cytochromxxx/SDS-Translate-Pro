...
class MemoryLogHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        # Ignoriere irrelevante Polling-Requests (Spam-Filter)
        if "GET /api/logs" in msg or "GET /api/check-import-session" in msg:
            return
        self.logs.append(msg)

memory_handler = MemoryLogHandler(capacity=1000)
memory_handler.setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(name)s - %(message)s', datefmt='%H:%M:%S'))
...
# Leite alle print()-Befehle an den Logger um, damit sie im Web-Terminal sichtbar werden
...
sys.stdout = StreamToLogger('SYSTEM')
sys.stderr = StreamToLogger('SYSTEM_ERR', logging.ERROR)

# Add msys2 GTK path to system PATH for WeasyPrint
msys2_path = r'C:\msys64\mingw64\bin'
if os.path.isdir(msys2_path) and hasattr(os, "add_dll_directory"):
...
app = Flask(__name__)
app.config['MEMORY_LOGS'] = memory_handler.logs
