...
def import_sds_to_html(
    ...
    try:
        ...
        if pdf_path and os.path.exists(pdf_path):
            try:
                from pdf_section_extractor import extract_sections_from_pdf, parse_section_16, parse_section_12, parse_section_1, parse_section_2, parse_section_8, parse_section_15, parse_section_3
                ...
                if pdf_sections:
                    if pdf_sections.get('section_1') and 'section_1' in sds_data:
                        ...
                        if section_1_data.get('lcs'):
                            if 'relevant_uses' not in sds_data['section_1']:
                                sds_data['section_1']['relevant_uses'] = {}
                            sds_data['section_1']['relevant_uses']['lcs'] = section_1_data['lcs']
                ...
                # Fill Section 3 gaps from PDF
                if pdf_sections and pdf_sections.get('section_3'):
                    s3_limits = parse_section_3(pdf_sections.get('section_3', ''))
                    if s3_limits and 'section_3' in sds_data and 'mixture_components' in sds_data['section_3']:
                        for comp in sds_data['section_3']['mixture_components']:
                            for cas in s3_limits:
                                if comp.get('cas') and cas in str(comp['cas']):
                                    comp['workplace_limit'] = "Substance with a community workplace exposure limit."
                        logger.info("Section 3 workplace limits filled from PDF")
                ...
                # Fill Section 8 gaps from PDF
                if pdf_sections and pdf_sections.get('section_8'):
                    section_8_data = parse_section_8(pdf_sections.get('section_8', ''))
                    if section_8_data.get('occupational_exposure_limits'):
                        if 'section_8' not in sds_data:
                            sds_data['section_8'] = {}
                        sds_data['section_8']['occupational_exposure_limits'] = section_8_data['occupational_exposure_limits']
                        logger.info("Section 8 limits filled from PDF")
                ...
                if pdf_sections and pdf_sections.get('section_12'):
                    ...
                    if section_12_data.get('persistence_and_degradability'):
                        ...
                        for cas_no, comp_data in bio_data.items():
                            found = False
                            for comp in sds_data['section_12'].get('ecotox_components', []):
                                if comp.get('cas_no') == cas_no or comp_data.get('name', '').lower() in comp.get('generic_name', '').lower() or comp.get('generic_name', '').lower() in comp_data.get('name', '').lower():
                            ...
                            if not found:
                                ...
                                sds_data['section_12']['ecotox_components'].append({
                                    'generic_name': comp_data.get('name', cas_no),
                                    'cas_no': cas_no,
                        ...
                        for cas_no, comp_data in bio_data.items():
                            ...
                            if not found:
                                ...
                                sds_data['section_12']['ecotox_components'].append({
                                    'generic_name': comp_data.get('name', cas_no),
                                    'cas_no': cas_no,
                                    'log_kow': comp_data.get('log_kow', ''),
                                    'bcf': comp_data.get('bcf', '')
                                })
                    ...
                    if section_12_data.get('pbt_vpvb_assessment'):
                        ...
                        for cas_no, comp_data in pbt_data.items():
                            found = False
                            for comp in sds_data['section_12'].get('ecotox_components', []):
                                if comp.get('cas_no') == cas_no or comp_data.get('name', '').lower() in comp.get('generic_name', '').lower() or comp.get('generic_name', '').lower() in comp_data.get('name', '').lower():
                            ...
                            if not found:
                                ...
                                sds_data['section_12']['ecotox_components'].append({
                                    'generic_name': comp_data.get('name', cas_no),
                                    'cas_no': cas_no,
                                    'pbt_result': comp_data.get('assessment', '')
                                })
                    ...
                    # Fill Section 15 gaps from PDF
                    if pdf_sections and pdf_sections.get('section_15'):
                        section_15_data = parse_section_15(pdf_sections.get('section_15', ''))
                        if 'section_15' not in sds_data:
                            sds_data['section_15'] = {}
                        if section_15_data.get('eu_legislation') and not sds_data['section_15'].get('eu_legislation'):
                            sds_data['section_15']['eu_legislation'] = section_15_data['eu_legislation']
                        if section_15_data.get('restrictions'):
                            sds_data['section_15']['restrictions'] = section_15_data['restrictions']
                        if section_15_data.get('stoerfallverordnung'):
                            sds_data['section_15']['stoerfallverordnung'] = section_15_data['stoerfallverordnung']
                        if section_15_data.get('betriebssicherheitsverordnung'):
                            sds_data['section_15']['betriebssicherheitsverordnung'] = section_15_data['betriebssicherheitsverordnung']
                        if section_15_data.get('wgk'):
                            sds_data['section_15']['wgk'] = section_15_data['wgk']
                        logger.info("Section 15 gaps filled from PDF")
