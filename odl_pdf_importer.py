...
def _parse_with_opendataloader(sds_data: Dict[str, Any], pdf_path: str) -> Dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as temp_output_dir:
        ...
        # Save debug output
        debug_path = os.path.join(temp_output_dir, os.path.basename(pdf_path) + ".odl_debug.json")
...
def _fill_gaps_from_pdfplumber(sds_data: Dict[str, Any], pdf_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "section_8" in sds_data and pdf_data.get("tables"):
        ...
        if any(keyword in all_text for keyword in ["section 8", "abschnitt 8", "exposure", "exposition", "oel", "peel", "mak", "agw"]):
            logger.info("[Gap-Filler] Tabellen für Sektion 8 gefunden")
            # TODO: Implementiere hier die echte Tabellen-Extraktions- und Mapping-Logik
...
def _fill_gaps_from_markdown(sds_data: Dict[str, Any], markdown_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # TODO: Implementiere hier die Logik, um die MD-Daten (Chandra) in sds_data zu injizieren
    return sds_data
...
def _fill_gaps_from_json(sds_data: Dict[str, Any], json_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # TODO: Implementiere hier die Logik, um Lücken in sds_data basierend auf json_data zu füllen
    return sds_data