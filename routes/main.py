        ...
        return jsonify(
            {
                ...
                "coverage": translator.stats["translated_exact"]
                / max(translator.stats["total_texts"], 1)
                * 100,
                "not_found": not_found,
...
def export_translated_html(html_content, target_lang, lang_name):
    ...
    if os.path.exists("mb_logo.svg"):
        ...
        html_content = re.sub(
            r'src=["\']?(?:mb_logo\.svg)["\']?', f'src="{logo_data_uri}"', html_content
        )

    # Find all ghs image references in HTML using regex
    ghs_refs = re.findall(r'src=["\']?ghs/([^"\']+\.png)["\']?', html_content)

    # Process GHS images - check aliases first, then fall back to ghs folder
    for ghs_file in set(ghs_refs):
        data_uri = None

        # Check if this file is in our aliases dictionary
        if ghs_file in aliases:
            symbol_file = aliases[ghs_file]
            symbol_path = os.path.join("symbole", symbol_file)
            if os.path.exists(symbol_path):
                data_uri = image_to_data_uri(symbol_path)

        # Fall back to ghs folder if no alias match or file doesn't exist
        if not data_uri:
            ghs_path = os.path.join("ghs", ghs_file)
            if os.path.exists(ghs_path):
                data_uri = image_to_data_uri(ghs_path)
    ...
    transport_dir = os.path.join("symbole", "transport")
    if os.path.exists(transport_dir):
        for t_file in os.listdir(transport_dir):
            if t_file.endswith(".png"):
                ...
                html_content = re.sub(
                    rf'src=["\']?symbole/transport/{re.escape(t_file)}["\']?',
                    f'src="{t_data_uri}"',
                    html_content,
                )
...
{html_content}
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=translated_document_{datetime.now().strftime('%Y%m%d')}.html"
    )
    return response
...
def export_translated_pdf(html_content, target_lang, lang_name):
    ...
    try:
        ...
        if os.path.exists(ghs_src):
            ...
            # Copy alias files from symbole/ folder to ghs folder
            symbole_dir = "symbole"
            for alias_name, symbol_file in aliases.items():
                symbol_path = os.path.join(symbole_dir, symbol_file)
                if os.path.exists(symbol_path):
                    # Copy with alias name to ghs folder
                    alias_dest = os.path.join(ghs_dest, alias_name)
                    shutil.copy2(symbol_path, alias_dest)

        # Copy transport folder from symbole/transport if it exists
        transport_src = os.path.join("symbole", "transport")
        transport_dest = os.path.join(temp_dir, "symbole", "transport")
        if os.path.exists(transport_src):
            os.makedirs(os.path.dirname(transport_dest), exist_ok=True)
            shutil.copytree(transport_src, transport_dest)

        # Legacy: Also check for transport folder at root level
        transport_root_src = "transport"
        transport_root_dest = os.path.join(temp_dir, "transport")
        if os.path.exists(transport_root_src) and not os.path.exists(transport_dest):
            shutil.copytree(transport_root_src, transport_root_dest)
        ...
        response.headers["Content-Disposition"] = (
            f"attachment; filename=translated_document_{datetime.now().strftime('%Y%m%d')}.pdf"
        )
        return response
...
def _prepare_html_for_export(html_content):
    ...
    html_content = re.sub(
        r'src=["\']mb_logo\.svg["\']', f'src="{base_url}/mb_logo.svg"', html_content
    )
    html_content = re.sub(
        r'src=["\']/?mb_logo\.svg["\']', f'src="{base_url}/mb_logo.svg"', html_content
    )
    ...
    ghs_pattern = re.compile(
        r'<div class="ghs-placeholder" data-symbol="(GHS\d+)">[\s\S]*?</div>'
    )
    ...
    html_content = re.sub(
        r'src=["\']/?(ghs/[^"\']+)["\']', f'src="{base_url}/\1"', html_content
    )

    # Handle relative transport image paths
    html_content = re.sub(
        r'src=["\']/?(transport/[^"\']+)["\']', f'src="{base_url}/\1"', html_content
    )

    # Handle relative static paths
    html_content = re.sub(
        r'src=["\']/?static/([^"\']+)["\']', f'src="{base_url}/static/\1"', html_content
    )
...
def export_translated_docx(html_content, target_lang, lang_name):
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=translated_document_{datetime.now().strftime('%Y%m%d')}.docx"
    )
    return response
...
def process_sdscom_xml():
    ...
    temp_xml_path = os.path.join(
        upload_folder, f"xml_{os.urandom(8).hex()}_{secure_filename(file.filename)}"
    )
    ...
    try:
        ...
        rendered_filename = (
            f"imported_{os.path.splitext(secure_filename(file.filename))[0]}.html"
        )
        ...
        return jsonify(
            {"success": True, "filename": file.filename, "preview": rendered_html}
        )
...
def process_combined_import():
    ...
    pdf_engine = request.form.get("pdf_engine", "standard")
    ...
    temp_xml_path = os.path.join(
        upload_folder, f"xml_{os.urandom(8).hex()}_{secure_filename(xml_file.filename)}"
    )
    temp_pdf_path = os.path.join(
        upload_folder, f"pdf_{os.urandom(8).hex()}_{secure_filename(pdf_file.filename)}"
    )
    ...
    try:
        ...
        # Wende die ausgewählte PDF Engine an
                    ...
                    for cls in component["classification"]:
                        ...
                        if ":" in cls:
                            ...
                            translated_classifications.append(
                                f"{translated_category}: {text}"
                            )
                        else:
                            translated_classifications.append(
                                translate_classification(cls)
                            )
...
def save_template():
    ...
    for pattern in suspicious_patterns:
        if pattern.lower() in content.lower():
            return jsonify(
                {"success": False, "error": "Verdächtige Inhalte erkannt: " + pattern}
            )

    template_path = os.path.join(current_app.root_path, "SDS_PERFEKT_TEMPLATE.html")
    backup_path = os.path.join(
        current_app.root_path, "SDS_PERFEKT_TEMPLATE.backup.html"
    )
    try:
...
def reset_template():
    ...
    try:
        ...
        if os.path.exists(template_path):
            ...
            return jsonify(
                {"success": False, "error": "Template-Datei nicht gefunden."}
            )
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
...
def get_mappings():
    ...
    try:
        if os.path.exists(mappings_path):
            ...
            return jsonify({"success": True, "mappings": default_mappings})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
    ...
    for m in mappings:
        ...
        if "templateVar" not in m or "sourceField" not in m:
            return jsonify(
                {
                    "success": False,
                    "error": "Mapping muss templateVar und sourceField enthalten",
                }
            )

    mappings_path = os.path.join(current_app.root_path, "mappings.json")
    backup_path = os.path.join(current_app.root_path, "mappings.backup.json")
    try:
        import shutil
        if os.path.exists(mappings_path):
...
@main_bp.route("/api/mappings/reset", methods=["POST"])
def reset_mappings():
    """Reset template field mappings to backup."""
    import json
    import shutil
    ...
    try:
        if os.path.exists(backup_path):
            shutil.copy(backup_path, mappings_path)
        ...
        if os.path.exists(mappings_path):
            ...
            return jsonify({"success": False, "error": "Keine Mappings gefunden."})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})

@main_bp.route("/api/mappings/scan_template", methods=["GET"])
def scan_template():
    """Extract Jinja template variables and loops from the template file."""
    import re
    template_path = os.path.join(current_app.root_path, "SDS_PERFEKT_TEMPLATE.html")
    ...
    try:
        ...
        # Find {{ variables }}
        vars_matches = re.findall(r'\{\{\s*([\w\.]+)(?:\s*\|.*?)?\s*\}\}', content)
        
        # Find {% for ... in array %}
        loops_matches = re.findall(r'\{%\s*for\s+[\w]+\s+in\s+([\w\.]+)\s*%\}', content)
        
        # Combine and deduplicate
        all_vars = set(vars_matches + loops_matches)
        
        # Group by section based on prefix (e.g. "section_1.product_name" -> "section_1")
        sections = {}
        for v in all_vars:
            parts = v.split('.')
            section = parts[0] if len(parts) > 1 else "global"
        ...
        return jsonify({"success": True, "variables": sections})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})

@main_bp.route("/api/mappings/scan_reference", methods=["POST"])
def scan_reference():
    """Scan an XML/JSON file and return a path-value tree."""
    if "file" not in request.files:
    ...
    try:
        ...
        if filename.endswith(".json"):
            ...
            paths = _flatten_dict(data)
            return jsonify({"success": True, "paths": paths})
            
        elif filename.endswith(".xml"):
            # Simple regex-based XML structure extraction for mapping UI purposes
            import re
            ...
            try:
                ...
                paths = {}
                _extract_xml_paths(root, "", paths)
                return jsonify({"success": True, "paths": paths})
            except Exception as parse_e:
                print(f"XML parse error for reference: {parse_e}")
                return jsonify({"success": False, "error": f"Fehler beim XML Parsen: {str(parse_e)}"})
        else:
            return jsonify({"success": False, "error": "Nur XML oder JSON Dateien werden unterstützt."})
...
def _flatten_dict(d, parent_key='', sep='/', paths=None):
    if paths is None:
        paths = {}
    if isinstance(d, dict):
        for k, v in d.items():
            new_key = f"{parent_key}{sep}{k}" if parent_key else k
            _flatten_dict(v, new_key, sep, paths)
    elif isinstance(d, list):
        # Sample first item for lists
        if len(d) > 0:
            if isinstance(d[0], (dict, list)):
                _flatten_dict(d[0], parent_key, sep, paths)
            else:
                paths[parent_key] = str(d[0])
    else:
        paths[parent_key] = str(d)
    return paths

def _extract_xml_paths(elem, current_path, paths):
    # Determine the tag name without namespace
    tag = elem.tag.split('}')[-1] if '}' in elem.tag else elem.tag
    path = f"{current_path}/{tag}" if current_path else tag
    
    # Store text content if present and not just whitespace
    if elem.text and elem.text.strip():
        # Only keep the first sample we find for a path
        if path not in paths:
            paths[path] = elem.text.strip()
            
    # Process children
    for child in elem:
        _extract_xml_paths(child, path, paths)
    
    # If it's an empty node without children but has attributes, maybe store attributes
    if len(elem) == 0 and not (elem.text and elem.text.strip()) and path not in paths:
        paths[path] = ""


@main_bp.route("/api/logs", methods=["GET"])
def get_logs():
    """Retrieve application logs for the terminal."""
    logs = current_app.config.get('MEMORY_LOGS', [])
...
def check_import_session():
    ...
    session_file = os.path.join(
        current_app.config["UPLOAD_FOLDER"], "import_session.json"
    )

    # DEBUG: Logging hinzufügen
    print(f"DEBUG: Checking for session file at: {session_file}")
    print(
        f"DEBUG: UPLOAD_FOLDER config: {current_app.config.get('UPLOAD_FOLDER', 'NOT_SET')}"
    )
...
@main_bp.route("/api/symbols/list", methods=["GET"])
def list_symbols():
    """Gibt alle verfügbaren Symbole (GHS, PSA, Transport) als Base64 für den Editor zurück."""
    import os
    import base64
    ...
    psa = []
    for p in [
        {"file": "M004_Augenschutz-benutzen.jpg", "name": "Augenschutz"},
        {"file": "M009_Handschutz_benutzen.jpg", "name": "Handschutz"},
        {"file": "M017_Atemschutz-benutzen.jpg", "name": "Atemschutz"},
        {"file": "M010_Schutzkleidung-benutzen.jpg", "name": "Schutzkleidung"}
    ]:
        b64 = get_b64(os.path.join("symbole", p["file"]))
        if b64:
            psa.append({"file": p["file"], "name": p["name"], "b64": b64})
            
    transport = []
    transport_dir = os.path.join("symbole", "transport")
    if os.path.exists(transport_dir):
        for f in os.listdir(transport_dir):
            if f.lower().endswith(('.png', '.jpg', '.jpeg', '.gif')):
                b64 = get_b64(os.path.join(transport_dir, f))
                if b64:
                    transport.append({"file": f, "name": os.path.splitext(f)[0], "b64": b64})
                    
    ghs = []
    ghs_dir = "ghs"
    if os.path.exists(ghs_dir):
        for f in os.listdir(ghs_dir):
             if f.lower().endswith('.png'):
                 b64 = get_b64(os.path.join(ghs_dir, f))
                 if b64:
                     ghs.append({"file": f, "name": os.path.splitext(f)[0].upper(), "b64": b64})
                     
    return jsonify({"psa": psa, "transport": transport, "ghs": ghs})
