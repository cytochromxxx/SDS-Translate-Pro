...
def import_sds_json_to_html(
    ...
    # 4. Inject top-level aliases expected by the template
    sec16_info = sds_data.get('section_16', {}).get('other_information', {})
    sds_data['other_information'] = sec16_info or {
        'indication_of_changes': [], 'abbreviations': [],
        'literature_references': '', 'training_advice': '', 'additional_info_lines': []
    }
    sds_data['hazard_identification'] = {
        'clp_classifications': sds_data.get('section_2', {}).get('classification', [])
    }
