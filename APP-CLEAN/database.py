...
CSV_TRANSLATION_SOURCE = os.path.join(BASE_DIR, "sds_phrases_PERFEKT.csv")
CSV_TRANSLATION_SOURCE_FALLBACK = os.path.join(os.path.dirname(BASE_DIR), "sds_phrases_PERFEKT.csv")
CSV_TRANSLATION_DB_PATH = os.path.join(BASE_DIR, "sds_phrases_PERFEKT.db")
_current_db_path = CSV_TRANSLATION_DB_PATH


def _get_translation_csv_path():
    if os.path.exists(CSV_TRANSLATION_SOURCE):
        return CSV_TRANSLATION_SOURCE
    if os.path.exists(CSV_TRANSLATION_SOURCE_FALLBACK):
        return CSV_TRANSLATION_SOURCE_FALLBACK
    return None
...
def ensure_translation_database_ready():
    csv_path = _get_translation_csv_path()
    if not csv_path:
        return
