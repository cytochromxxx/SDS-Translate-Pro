...
def export_translated_pdf(html_content, target_lang, lang_name):
    ...
    try:
        ...
        try:
            # Modern PDF Export using Playwright (Pixel-Perfect Chromium rendering)
            ...
            with sync_playwright() as p:
                ...
                # Wait for any network requests/images to load
                page.wait_for_load_state("networkidle")
                
                # Ensure all web fonts are fully loaded for accurate text measuring
                page.evaluate("document.fonts.ready")
                
                # Inject Print-CSS to perfectly preserve table layouts and prevent ugly page breaks
                ...
                pdf_bytes = page.pdf(
                    ...
                    footer_template="<div style='font-size: 9px; color: #555; text-align: center; width: 100%; font-family: Arial, sans-serif;'>Seite <span class='pageNumber'></span> von <span class='totalPages'></span></div>"
                )
                browser.close()
        except ImportError:
            # Fallback to WeasyPrint if Playwright is not installed
            ...
            print("Warning: Used WeasyPrint for PDF generation. For better layout, install playwright.")
