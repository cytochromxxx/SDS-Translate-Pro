...
class SDSPDFGapFiller:
    ...
    def extract_ppe_icons_list(self) -> List[str]:
        ...
        b64_icons = []
        for icon in icons:
            b64 = self._get_symbol_as_base64(icon)
            if b64:
                b64_icons.append(b64)
        return b64_icons

    def extract_section_8_ppe(self) -> List[str]:
        ...
        try:
            ...
            if m:
                ...
                if not changes:
                    ...
                    if plain_text and plain_text.lower() not in ["none", "no data available"]:
                        changes.append(plain_text)
        except Exception as e:
    ...
    def _extract_abbreviations(self):
        ...
        def _is_hazard_statement(s):
            ...
            for keyword in hazard_keywords:
                if keyword in s_lower and len(s_stripped) < 200:  # Hazard statements are typically shorter
                    return True
            return False
        
        def _is_valid_abbrev(s):
            # Exclude H-codes (H-statements) — they belong to hazard statements, not abbreviations
            # Enhanced check for various H-code formats
            if re.search(r'\bH\s*[-]?\d{3}\b', s):
                return False
            # Exclude strings that look like hazard statement descriptions
            if _is_hazard_statement(s):
                return False
            if not (2 <= len(s) <= 20):
                return False
            if s.endswith(','):
        ...
        try:
            ...
            if note_match:
                source_note = " ".join(note_match.group(1).split())
            ...
            for page_idx in range(8, 12):
                ...
                for table in tables:
                    ...
                    # Check table header to verify it's an abbreviations table
                    header = table[0]
                    header_str = " ".join(str(c) for c in header if c).lower() if header else ""
                    
                    # Skip if header indicates this is a hazard statements table
                    if "hazard statement" in header_str or "h code" in header_str or "h number" in header_str:
                        continue
                    
                    # Only process tables that look like they contain abbreviations
                    # An abbreviations table should have short codes/acronyms in first column
                    valid_rows = []
                    hazard_rows_found = 0  # Track if we find hazard statement rows
                    
                    for row in table:
                        if not row or len(row) < 2: 
                            continue
                        c0, c1 = str(row[0] or "").strip(), str(row[1] or "").strip()
                        
                        # Exclude rows that contain hazard statements
                        if _is_hazard_statement(c0) or _is_hazard_statement(c1):
                            hazard_rows_found += 1
                            continue
                        
                        if _is_valid_abbrev(c0) and len(c1) > 3:
                            valid_rows.append({"short": c0, "long": c1})
                    
                    # Only accept the table if:
                    # 1. We found at least 3 valid abbreviations AND
                    # 2. We didn't find too many hazard statement rows (which would indicate this is the wrong table)
                    if len(valid_rows) >= 3 and hazard_rows_found < len(valid_rows) / 2:
                        for vr in valid_rows:
                            if vr not in abbreviations:
                                abbreviations.append(vr)
            
            # 3. Fallback: Parse from text if table extraction failed or missed some
            # Use stricter regex to find the exact abbreviations section
            m = re.search(r'16\.2\.?[^\n]*?(?:Abbreviation|acronym)[^\n]*?\n(.*?)(?=\n16\.3|\nFor abbreviation|\nSUBSTANCE|\nSubstance|\Z)', text, re.DOTALL | re.IGNORECASE)
            if m and not abbreviations:
                block = m.group(1)
                # Filter out lines that contain hazard statement patterns
                lines = [l for l in block.splitlines() if l.strip() and not _is_hazard_statement(l)]
                ...
                for i, line in enumerate(lines):
                    ...
                    # Try splitting by 2+ spaces first (visually aligned columns)
                    parts = re.split(r'\s{2,}', line, 1)
                    if len(parts) == 2:
                        short, long = parts[0].strip(), parts[1].strip()
                        if _is_valid_abbrev(short) and not _is_hazard_statement(long):
                            abbreviations.append({"short": short, "long": long})
                            continue
                            
                    # Try splitting by 1 space
                    parts = line.split(' ', 1)
                    if len(parts) == 2:
                        short, long = parts[0].strip(), parts[1].strip()
                        # Strict check to avoid capturing normal sentences
                        if _is_valid_abbrev(short) and len(long) > 3 and not _is_hazard_statement(long):
                            abbreviations.append({"short": short, "long": long})
                            continue
                            
                    # Try interleaved lines (Short on one line, Long on next)
                    if len(parts) == 1 and _is_valid_abbrev(line):
                        if i + 1 < len(lines):
                            next_line = lines[i+1].strip()
                            if len(next_line) > 3 and not _is_valid_abbrev(next_line.split(' ')[0]) and not _is_hazard_statement(next_line):
                                abbreviations.append({"short": line, "long": next_line})
    ...
    def _extract_literature_references(self) -> str:
        ...
        try:
            ...
            if m:
                block = m.group(1)
                block = re.split(r'(?i)Substance name|Stoffname', block)[0].strip()
                lines = []
                for line in block.splitlines():
                    line = line.strip()
                    if line and not re.match(r'^\d+/\d+$', line):
                        lines.append(line)
                return "<br>\n".join(lines)
        except Exception as e:
        ...
        try:
            ...
            for page_idx in range(8, 12):
                ...
                for table in tables:
                    ...
                    # CLP classification table should have 3 columns:
                    # hazard class, hazard statement, classification procedure
                    if len(header) < 3:
                        continue  # Skip tables that don't have 3 columns
                    ...
                    for row in table[1:]:
                        if not row or all(c is None or str(c).strip() == "" for c in row):
                            continue
                        cells = [str(c).strip() if c else "" for c in row]
                        # Only add rows with 3 cells (hazard_class, hazard_statement, procedure)
                        if len(cells) >= 3:
                            results.append({
                                "hazard_class": cells[0],
                                "hazard_statement": cells[1],
                                "procedure": cells[2],
                            })
                    if results: 
                        return results # Stop after finding the first valid table
        except Exception as e:
        ...
        try:
            ...
            for page_idx in range(8, 12):
                ...
                for table in tables:
                    ...
                    # Only extract from tables that explicitly contain hazard statements
                    # Look for specific markers that indicate this is section 16.5
                    if "hazard statement" not in header_str:
                        continue
                    
                    # Additional check: this should be a 2-column table (H-code, description)
                    # 3+ columns typically indicate a classification table, not a pure hazard statements list
                    if len(header) >= 3:
                        # Could be a classification table - verify by checking if we can find actual H-codes
                        continue
                    ...
                    for row in table[1:]:
                        ...
                        # Strict validation: first cell must be an H-code in standard format
                        if len(cells) >= 2 and re.match(r'^H\d{3}(?:\s|:|$)', cells[0]):
                            results.append({"code": cells[0].strip(), "text": cells[1]})
    ...
    def _extract_training_advice(self) -> str:
        ...
        try:
            ...
            if m:
                lines = [l.strip() for l in m.group(1).splitlines() if l.strip() and not re.match(r'^\d+/\d+$', l.strip())]
                val = "<br>\n".join(lines)
                if "no data available" not in val.lower():
                    return val
        except Exception as e:
    ...
    def extract_section_9_properties(self) -> Dict[str, str]:
        """Extracts basic physical and chemical properties from Section 9."""
        result = {}
        try:
            text = self._get_section_text("9.1", "10.")
            if not text:
                text = self._get_section_text("SECTION 9", "SECTION 10")
            
            state_match = re.search(r'(?:Physical state|Aggregatzustand|Form)[:\s]+([^\n]+)', text, re.I)
            if state_match: result['physical_state'] = state_match.group(1).strip()
            
            color_match = re.search(r'(?:Colour|Color|Farbe)[:\s]+([^\n]+)', text, re.I)
            if color_match: result['colour'] = color_match.group(1).strip()
            
            ph_match = re.search(r'pH(?:-Value)?[:\s]+([\d\.,]+)', text, re.I)
            if ph_match: result['ph'] = ph_match.group(1).strip()
            
            flash_match = re.search(r'(?:Flash point|Flammpunkt)[:\s]+([\d\.,]+\s*°C)', text, re.I)
            if flash_match: result['flash_point'] = flash_match.group(1).strip()
        except Exception as e:
            logger.warning(f"Could not extract Section 9 properties: {e}")
        return result
    ...
    def extract_section_10(self) -> Dict[str, str]:
        """Extracts text for reactivity, stability, and incompatible materials from Section 10."""
        result = {}
        try:
            text = self._get_section_text("10.1", "11.")
            if not text:
                text = self._get_section_text("SECTION 10", "SECTION 11")
            
            react_match = re.search(r'10\.1\.?\s*Reactivity\s*\n(.*?)(?=10\.2|\Z)', text, re.S|re.I)
            if react_match: result['reactivity'] = " ".join(react_match.group(1).strip().split())
            
            stab_match = re.search(r'10\.2\.?\s*Chemical stability\s*\n(.*?)(?=10\.3|\Z)', text, re.S|re.I)
            if stab_match: result['chemical_stability'] = " ".join(stab_match.group(1).strip().split())
            
            cond_match = re.search(r'10\.4\.?\s*Conditions to avoid\s*\n(.*?)(?=10\.5|\Z)', text, re.S|re.I)
            if cond_match: result['conditions_to_avoid'] = " ".join(cond_match.group(1).strip().split())
            
            incomp_match = re.search(r'10\.5\.?\s*Incompatible materials\s*\n(.*?)(?=10\.6|\Z)', text, re.S|re.I)
            if incomp_match: result['incompatible_materials'] = " ".join(incomp_match.group(1).strip().split())
        except Exception as e:
            logger.warning(f"Could not extract Section 10 gaps: {e}")
        return result

    def extract_section_13_waste_treatment(self) -> str:
        """Extracts the general waste treatment methods from Section 13.1."""
        try:
            text = self._get_section_text("13.1", "14.")
            if not text:
                text = self._get_section_text("SECTION 13", "SECTION 14")
                
            match = re.search(r'13\.1\.?\s*Waste treatment methods\s*\n(.*?)(?=Waste code|13\.2|\Z)', text, re.S|re.I)
            if match:
                return " ".join(match.group(1).strip().split())
        except Exception as e:
            logger.warning(f"Could not extract Section 13 waste treatment methods: {e}")
        return ""
    ...
    def extract_section_14_classes(self) -> Dict[str, str]:
        ...
        try:
            ...
            inland_match = re.search(r'ADN.*?Class(?:es)?:\s*([\d\.]+)', text, re.S|re.I)
            if inland_match: result['inland'] = inland_match.group(1).strip()
            
            sea_match = re.search(r'IMDG.*?Class(?:es)?:\s*([\d\.]+)', text, re.S|re.I)
            if sea_match: result['sea'] = sea_match.group(1).strip()
    ...
    def fill_gaps(self, parsed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            prod_id = sec1.get("product_identifier", {})
            if _is_empty(prod_id.get("ufi")):
                text = "\n".join([self._page_text(i) for i in range(min(3, len(self._pdf.pages) if self._pdf else 0))])
                ufi_match = re.search(r'UFI:\s*([A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4})', text, re.IGNORECASE)
                if ufi_match:
                    prod_id["ufi"] = ufi_match.group(1).upper()
                    if "product_identifier" not in sec1:
                        sec1["product_identifier"] = prod_id
                    data["section_1"] = sec1
                    logger.info("Filled Section 1 UFI from PDF")
        except Exception as e:
            logger.warning(f"Could not fill Section 1 UFI: {e}")
        ...
        try:
            ...
            if "environmental contamination" in self._full_text():
                ...
                if _is_empty(c_text):
                    sec6["containment"] = "Use appropriate container to avoid environmental contamination."
                else:
            ...
            if "exhaust ventilation" in t7 or "exhaust ventilation" in self._full_text():
                ...
                elif _is_empty(sh_text):
                    sec7["safe_handling"] = "If local exhaust ventilation is not possible or not sufficient, the entire working area should be ventilated by technical means."
                else:
            ...
            safety_data = sec9.get("safety_data", [])
            
            if "completely miscible" in t9 or "completely miscible" in self._full_text():
                if not any(item.get('parameter') == 'Water solubility' and 'completely miscible' in item.get('value', '') for item in safety_data):
                    safety_data.append({'parameter': 'Water solubility', 'value': 'completely miscible', 'temperature': '', 'method': ''})
                    
            # Extract additional physical properties
            pdf_s9 = self.extract_section_9_properties()
            
            # Appearance
            appearance = sec9.get("appearance", "")
            app_parts = []
            if pdf_s9.get("physical_state") and "state" not in appearance.lower() and "form" not in appearance.lower():
                app_parts.append(f"Physical state: {pdf_s9['physical_state']}")
            if pdf_s9.get("colour") and "color" not in appearance.lower() and "colour" not in appearance.lower() and "farbe" not in appearance.lower():
                app_parts.append(f"Color: {pdf_s9['colour']}")
            
            if app_parts:
                if appearance and not appearance.strip().endswith("<br>"):
                    appearance += "<br>"
                sec9["appearance"] = appearance + "<br>".join(app_parts)
                
            # Safety data (pH, Flash point)
            has_ph = any("ph" in item.get("parameter", "").lower() for item in safety_data)
            has_flash = any("flash" in item.get("parameter", "").lower() or "flamm" in item.get("parameter", "").lower() for item in safety_data)
            
            if pdf_s9.get("ph") and not has_ph:
                safety_data.append({"parameter": "pH", "value": pdf_s9["ph"], "temperature": "", "method": ""})
            if pdf_s9.get("flash_point") and not has_flash:
                safety_data.append({"parameter": "Flash point", "value": pdf_s9["flash_point"], "temperature": "", "method": ""})

            sec9["safety_data"] = safety_data
            data["section_9"] = sec9
            ...
            if "No hazardous reaction" in t10 or "No hazardous reaction" in self._full_text():
                hr_text = sec10.get("hazardous_reactions", "")
                if _is_empty(hr_text):
                    sec10["hazardous_reactions"] = "No hazardous reaction when handled and stored according to provisions."
                else:
                    if "No hazardous reaction" not in hr_text:
                        sec10["hazardous_reactions"] = "No hazardous reaction when handled and stored according to provisions. " + hr_text
            
            # Extract additional Section 10 gaps
            pdf_s10 = self.extract_section_10()
            for key in ["reactivity", "chemical_stability", "conditions_to_avoid", "incompatible_materials"]:
                if pdf_s10.get(key) and _is_empty(sec10.get(key)):
                    sec10[key] = pdf_s10[key]
            ...
            if "Evidence for disposal" in self._full_text():
                req_text = sec13.get("eu_requirements", "")
                if "Evidence for disposal" not in req_text:
                    if _is_empty(req_text):
                        sec13["eu_requirements"] = "*: Evidence for disposal must be provided."
                    else:
                        sec13["eu_requirements"] = req_text + " *: Evidence for disposal must be provided."
        ...
        # --- Section 13: Waste Codes & Treatment ---
        try:
            sec13 = data.get("section_13", {})
            changed = False
            ...
            if waste_codes:
                if _is_empty(sec13.get("waste_code_product")) and 'product' in waste_codes:
                    ...
                    if "ewl_data" in sec13 and len(sec13["ewl_data"]) > 0:
                        sec13["ewl_data"][0]["code"] = waste_codes['product']
                    changed = True
                if _is_empty(sec13.get("waste_code_packaging")) and 'packaging' in waste_codes:
                    sec13["waste_code_packaging"] = waste_codes['packaging']
                    changed = True
                    
            waste_treatment = self.extract_section_13_waste_treatment()
            if waste_treatment and _is_empty(sec13.get("waste_treatment")):
                sec13["waste_treatment"] = waste_treatment
                changed = True

            if changed:
                data["section_13"] = sec13
                logger.info("Filled Section 13 gaps from PDF")
        except Exception as e:
            logger.warning(f"Could not fill Section 13 gaps: {e}")

        # --- Section 14: Transport Classes ---
        try:
            sec14 = data.get("section_14", {})
            if sec14:
                needs_filling = False
                for mode in ['land', 'inland', 'sea', 'air']:
                    if mode in sec14 and _is_empty(sec14[mode].get("transport_class")):
                        needs_filling = True
                        break
                        
                if needs_filling:
                    pdf_classes = self.extract_section_14_classes()
                    changed = False
                    for mode in ['land', 'inland', 'sea', 'air']:
                        if mode in sec14 and mode in pdf_classes and _is_empty(sec14[mode].get("transport_class")):
                            sec14[mode]["transport_class"] = pdf_classes[mode]
                            changed = True
                    ...
                    if changed:
                        try:
                            ...
                            classes = [
                                sec14.get('land', {}).get('transport_class'),
                                sec14.get('inland', {}).get('transport_class'),
                                sec14.get('sea', {}).get('transport_class'),
                                sec14.get('air', {}).get('transport_class')
                            ]
                            sec14['transport_icons'] = parser_helper._get_transport_icons_from_classes(classes)
                        except Exception as e:
                            logger.warning(f"Could not update transport icons after filling gaps: {e}")
                            
                        data["section_14"] = sec14
                        logger.info("Filled Section 14 Transport Classes from PDF")
        except Exception as e:
            logger.warning(f"Could not fill Section 14 gaps: {e}")
